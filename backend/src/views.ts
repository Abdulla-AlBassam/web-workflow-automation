...
function renderEffort(meta: Meta, log: Record<string, unknown>[], hasSpec: boolean): string {
  ...
  return `<details class="fold" id="effort"${hasSpec ? '' : ' open'}>
    ...
    <div class="runrow" style="margin-top:10px">
      ...
      ${pill('i', '<p>Downloads one Markdown file: your goal, the script contract, the acceptance rules, the answer format, then the sanitised evidence: the route, typed values, marks, every page snapshot, the last page’s HTML and the captured calls in full.</p><p><b>Full</b> (up to 4 MB) suits an agent that reads files, such as Claude Code. <b>Chat-sized</b> (600 KB) fits a chat window; whatever the size cut is listed at the end of the file.</p><p>The export also fetches each visited page once, without cookies, and tells the model whether the results are in the plain response. It replays the calls that carried what you typed, without credentials, so the model is told which of them are gated, and a table header row marked by mistake is listed as ignored. Snapshots show whatever was on your screen. The file is saved as BRIEF.md in the session folder too.</p>', 'About the export')}
