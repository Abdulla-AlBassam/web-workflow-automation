...
- **Columns come from marks.** The operator's highlighted text is located in the outcome response at generate time; the spec stores paths, so a new input yields the same fields for its own data. Matching ignores bracketed reference markers (`[4]`, `[a]`), prefers an exact field over a containing one, and a field inside the record set over one elsewhere.
...
- **Repaired specs carry provenance.** A spec built by the LLM repair assistant has `repaired: { at, model, diagnosis, mode, feedback? }`; `mode` is `repair` (the analyser had refused) or `refine` (a run was flagged, `feedback` holding the operator's note). Its columns were located in the live response the proposal produced. Repaired specs are never regenerated.
